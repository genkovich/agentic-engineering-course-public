...
двигун (`run.py`, кроки git + seed_commit) робить git-репо й засаджує зміну в історію (HEAD~1..HEAD), щоб read-only
...
`check.py` асертить `git status`/`git diff` по `src/` порожній.
