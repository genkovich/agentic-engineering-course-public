# Starter: Go + chi/v5 Production-Ready Claude Code
...
1. Клонуй repo або скопіюй цю директорію у свій проект:
   ...
   cd agentic-engineering-course-public/modules/3-claude-code-setup/3.9-starters/go-chi

   # груповий потік (потрібен доступ)
   git clone https://github.com/genkovich/agentic-engineering-course.git
   cd agentic-engineering-course/modules/3-claude-code-setup/3.9-starters/go-chi
   ```
...
| `.claude/settings.local.json.example` | Шаблон для Local overrides (gitignored у твоєму проекті) |
| `.devcontainer/devcontainer.json` | VS Code devcontainer з cap_add NET_ADMIN/NET_RAW |
| `.devcontainer/Dockerfile` | golang:1.22 + Claude Code + iptables + ipset |
...
`permissions.allow` для основного workflow Go: `go test ./...`, `go build ./...`, `go vet ./...`, `go mod tidy`, `go fmt ./...`, `golangci-lint run`, `git diff/status/log`, `make verify*`.
...
- `network.allowedDomains`: api.anthropic.com, proxy.golang.org, sum.golang.org, github.com та інші необхідні. Все поза цим списком блокується.
...
Останній шар - мережна ізоляція через iptables всередині контейнера:
- `Dockerfile` додає iptables і ipset, ставить Node 20 для встановлення Claude Code CLI globально.
...
`.claude/settings.json` `permissions.allow` адаптуй під свій workflow. Базовий список покриває Go toolchain, додай специфічне для твоєї збірки:
...
| `make test` | unit тести проекту через `go test ./...` |
...
- **iptables без NET_ADMIN/NET_RAW не працює.** `runArgs` у `devcontainer.json` і `cap_add` у `docker-compose.yml` мусять бути присутні. `make verify-devcontainer` ловить цю помилку.
- **DNS обмежений до nameserver контейнера.** `init-firewall.sh` дозволяє UDP/TCP 53 тільки до IP з `/etc/resolv.conf` (зазвичай `127.0.0.11` для Docker embedded DNS). Це захист від обходу whitelist через arbitrary DNS resolver.
...
- **IPv6 блокується default-deny.** `ip6tables` мірорить `iptables` правила. AAAA records резолвляться у `claude-allowed-6` ipset.
- **VS Code Reopen in Container спершу довгий.** Перший build тягне golang:1.22, ставить Node 20 і `npm install -g claude-code`. ~5-10 хв. Наступні запуски швидкі завдяки cache layers.
- **`.claude/settings.local.json`** має бути у `.gitignore` свого проекту. У repo лежить тільки `.example` шаблон.
- **`go.sum` згенерується автоматично.** При першому `go mod tidy` всередині контейнера (або хоста) Go завантажить chi/v5 і запише суми у `go.sum`. Файл треба закомітити у git.
- **Non-root користувач `vscode`.** `golang:1.22` base image не має готового non-root user, тож Dockerfile створює `vscode` (uid 1000) з sudoers правом тільки на `init-firewall.sh`.
