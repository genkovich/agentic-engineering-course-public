# Starter: Rust + axum Production-Ready Claude Code

**Module:** 3 - Claude Code Setup
**Stack:** Rust stable + axum + tokio + tower
...
1. Клонуй repo або скопіюй цю директорію у свій проект:
   ...
   cd agentic-engineering-course-public/modules/3-claude-code-setup/3.9-starters/rust-axum

   # груповий потік (потрібен доступ)
   git clone https://github.com/genkovich/agentic-engineering-course.git
   cd agentic-engineering-course/modules/3-claude-code-setup/3.9-starters/rust-axum
   ```
...
| `.devcontainer/Dockerfile` | rust:1 + Node.js 20 + Claude Code + iptables + ipset |
...
`permissions.allow` для основного workflow Rust: `cargo test`, `cargo build`, `cargo check`, `cargo clippy`, `cargo fmt`, `cargo run`, `cargo update`, `cargo doc`, `git diff/status/log`, `make verify*`.
...
- `network.allowedDomains`: api.anthropic.com, crates.io, static.crates.io, index.crates.io, github.com та інші необхідні. Все поза цим списком блокується.
...
- `Dockerfile` додає iptables і ipset, ставить Node.js 20 і встановлює Claude Code globально через npm.
...
`.claude/settings.json` `permissions.allow` адаптуй під свій workflow. Базовий список покриває Rust, додай специфічне для твоєї збірки:
...
| `make test` | unit тести проекту через `cargo test` |
...
- **VS Code Reopen in Container спершу довгий.** Перший build тягне `rust:1`, Node.js 20 з NodeSource і `npm install -g claude-code`. ~5-10 хв. Наступні запуски швидкі завдяки cache layers.
- **`.claude/settings.local.json`** має бути у `.gitignore` свого проекту. У repo лежить тільки `.example` шаблон.
- **Cargo cache у томах.** `target/` і `~/.cargo/registry` змонтовані як volumes - інакше кожен `docker compose up` тягнув би залежності з нуля. На хості тримай свій локальний `target/` поза volume.
- **`crates.io` vs `index.crates.io`.** Cargo резолвить індекс через `index.crates.io`, а самі crate-архіви тягне з `static.crates.io`. Обидва домени мають бути у whitelist, інакше `cargo build` падатиме на резолюції.
- **Node.js потрібен лише для Claude Code.** Сам стек чистий Rust. Базовий image `rust:1`, поверх ставиться Node 20 з NodeSource винятково для `npm install -g @anthropic-ai/claude-code`.
