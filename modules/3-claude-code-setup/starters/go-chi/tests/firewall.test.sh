...
# 3. proxy.golang.org має бути доступний (whitelist для Go стеку)
echo "Test 3: proxy.golang.org (whitelist для Go) має пройти"
if curl -fsS --max-time 5 https://proxy.golang.org -o /dev/null; then
  echo "OK: proxy.golang.org доступний"
else
  echo "FAIL: proxy.golang.org заблокований - стекоспецифічний whitelist зламаний"
  exit 1
fi
echo ""
