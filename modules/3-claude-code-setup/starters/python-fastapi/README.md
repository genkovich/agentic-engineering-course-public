# Starter: Python + FastAPI Production-Ready Claude Code
...
| `.devcontainer/Dockerfile` | python:3.12-slim + Node 20 + Claude Code + iptables + ipset |
...
`permissions.allow` для основного workflow Python: `pytest`, `python -m pytest`, `pip install -r requirements*.txt`, `pip install -e .`, `ruff check`, `ruff format`, `mypy`, `uvicorn`, `git diff/status/log`, `make verify*`.
...
- `network.allowedDomains`: api.anthropic.com, pypi.org, files.pythonhosted.org, github.com та інші необхідні. Все поза цим списком блокується.
...
- `Dockerfile` стартує з `python:3.12-slim`, додає iptables і ipset, ставить Node 20 з NodeSource (потрібен щоб встановити Claude Code через npm), створює non-root юзера `python`.
...
`.claude/settings.json` `permissions.allow` адаптуй під свій workflow. Базовий список покриває Python, додай специфічне для твоєї збірки:

```json
"allow": [
  "Bash(pytest *)",
  "Bash(uvicorn src.main:app *)",
...
| `make test` | pytest для FastAPI app |
...
- **Node всередині Python образу.** Claude Code зараз ставиться через npm, тому у `Dockerfile` після `python:3.12-slim` ставимо ще Node 20 з NodeSource. Це додає ~150MB у image, але дає офіційний шлях встановлення Claude Code.
- **VS Code Reopen in Container спершу довгий.** Перший build тягне python:3.12-slim, Node 20 і npm install -g claude-code. ~5-10 хв. Наступні запуски швидкі завдяки cache layers.
- **`.claude/settings.local.json`** має бути у `.gitignore` свого проекту. У repo лежить тільки `.example` шаблон.
- **`pip install -e .[dev]`** для dev залежностей (pytest, ruff, mypy). Без `[dev]` отримаєш тільки runtime (FastAPI + uvicorn).
- **PYTHONPATH для тестів.** `pyproject.toml` має `[tool.pytest.ini_options] pythonpath = ["src"]`, тому `from main import app` у тестах працює без додаткових налаштувань.
