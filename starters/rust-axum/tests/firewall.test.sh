...
# 3. crates.io має бути доступний (whitelist для Rust стеку)
echo "Test 3: crates.io (whitelist для Rust) має пройти"
if curl -fsS --max-time 5 https://crates.io -o /dev/null; then
  echo "OK: crates.io доступний"
else
  echo "FAIL: crates.io заблокований - стекоспецифічний whitelist зламаний"
  exit 1
fi
echo ""
